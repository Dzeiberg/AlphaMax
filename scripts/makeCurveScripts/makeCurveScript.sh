...
cd ../
for ((loopNum=49999; loopNum>=1; loopNum--))
do
	...
	do
		...
		if test -f "$F";then
			...
			matlab  -nodisplay -nosplash - nodesktop -r "curves = makeCurves('data/syntheticParameters.mat',@(x,y)CurveConstructor(x,y,'useGPU',true), 'setNumberStart', "$jobNum", 'setNumberEnd', "$jobNum", 'savePath','data/curves_paramset_"$jobNum".mat','quiet',true);exit;" > scripts/logs/out_$jobNum.out &
			pids[${i}]=$!
		fi
	done
